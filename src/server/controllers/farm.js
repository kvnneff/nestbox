...
const get = (req, res, next) => {
  ...
  Farm.find(user_id, (err, location) => {
    if (err) return next(err)
...
const showUser = (req, res, next) => {
  ...
  const user = req.user
  Farm.findFarm(farm_id, (err, location) => {
    if (err) return next(err)
...
const destroy = (req, res, next) => {
  ...
  Farm.find(location_id, (err, location) => {
    if (err) return next(err)
    location.remove((err) => {
      if (err) return next(err)
...
const getRecent = (req, res, next) => {
  Farm.findRecent((err, collection) => {
    if (err) return next(err)
...
const findWithinRadius = (req, res, next) => {
  ...
  Farm.findWithinRadius(latitude, longitude, radius, (err, farms) => {
    if (err) return next(err)
...
const findByName = (req, res, next) => {
  ...
  Farm.findByName(name, (err, farms) => {
    if (err) return next(err)
    return res.render('search', { farms, query: req.query })
  })
}
